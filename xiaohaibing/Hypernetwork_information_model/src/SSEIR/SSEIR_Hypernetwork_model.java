...
public class SEIIR_Hypernetwork_model {
    public static void main(String[] args) {
        final int NUM_EXPERIMENTS = 50;
        final int NUM_STEPS = 500;
        final String OUTPUT_FILE_PATH = "D:\\桌面\\Hypernetwork_information_model\\SEIIR_output\\SEIIR_Hypernetwork_model.txt";
        double[][] cumulativeStateRatios = new double[NUM_STEPS][5];
        double[][] cumulativeStateRatios_a;
        int n; // 节点总数
        double[][] matrix_2section; // 邻接矩阵
        try {
            matrix_2section = readMatrixFromFile("D:\\桌面\\Hypernetwork_information_model\\output\\collaboration.txt");
        ...
        for (int experiment = 0; experiment < NUM_EXPERIMENTS; experiment++) {
            //将每一次的重复实验的结果都记录下来
            String out_file_path = "D:\\桌面\\Hypernetwork_information_model\\SSEIR_output1\\SEIIR_Hypernetwork_model" + experiment + ".txt";
            ...
            double it = 50; // I状态的节点总数
            double rt = 0; // R状态的节点总数

            cumulativeStateRatios_a = new double[NUM_STEPS][5];
            for (int k = 0; k < NUM_STEPS; k++) {
                ...
                cumulativeStateRatios_a[k][0] = si / n;
                cumulativeStateRatios_a[k][1] = sa / n;
                cumulativeStateRatios_a[k][2] = et / n;
                cumulativeStateRatios_a[k][3] = it / n;
                cumulativeStateRatios_a[k][4] = rt / n;
            }
            try (FileWriter writer = new FileWriter(out_file_path)) {
                for (int k = 0; k < NUM_STEPS; k++) {
                    double avgSi = cumulativeStateRatios_a[k][0] ;
                    double avgSa = cumulativeStateRatios_a[k][1] ;
                    double avgEt = cumulativeStateRatios_a[k][2] ;
                    double avgIt = cumulativeStateRatios_a[k][3] ;
                    double avgRt = cumulativeStateRatios_a[k][4] ;
                    if (k == 0) {
                        writer.write(k + "," + 0.695 + "," + 0.3+ "," + 0 + "," + 0.005 + "," + 0 + "\n");
                    } else {
                        writer.write(k  + "," + avgSi + "," + avgSa + "," + avgEt + "," + avgIt + "," + avgRt + "\n");
                    }
