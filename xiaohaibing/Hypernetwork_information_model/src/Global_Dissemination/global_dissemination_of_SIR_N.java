...
public class global_dissemination_of_SIR_N {
    public static void main(String[] args) {
        double a = 0.1;
        double b = 0.1;
        int n1000, n5000, n10000;
        double[][] matrix_AdjacencyMatrix1000, matrix_AdjacencyMatrix5000, matrix_AdjacencyMatrix10000;
        try {
        ...
        for (int p = 0; p < 3; p++) {
            ...
            double it = 1;
            double rt = 0;
            try (FileWriter writer = new FileWriter("D:\\桌面\\Hypernetwork_information_model\\output\\global_dissemination_of_SIR_Hypernetwork_N"+n+".txt")) {
                    ...
                    for (int i = 0; i < n; i++) { // 遍历每个节点
                        if (state[i][1] == 1) {
                            ...
                            for (int j = 0; j < n; j++) { // 寻找第i个节点的邻居节点
                                if (matrix[i][j] == 1 && state[j][1] == 0 && random.nextDouble() < a) {
                                    newInfections[j] = 1;
                                }
                            }
                        }
