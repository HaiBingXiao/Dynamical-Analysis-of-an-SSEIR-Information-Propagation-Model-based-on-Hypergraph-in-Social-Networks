...
public class SIS_Hypernetwork_model {
    public static void main(String[] args) {
        double a = 0.3;
        double b = 0.01;
        int n;
        double[][] matrix_AdjacencyMatrix;
        try {
            matrix_AdjacencyMatrix = readMatrixFromFile("D:\\IDEA\\Hypernetwork_information_model\\output\\Hypernetwork_43_AdjacencyMatrix.txt");
        ...
        try (FileWriter writer = new FileWriter("D:\\IDEA\\Hypernetwork_information_model\\output\\SIS_Hypernetwork_model.txt")) {
                ...
                for (int i = 0; i < n; i++) { // 遍历每个节点
                    if (state[i][1] == 1) {
                        ...
                        for (int j = 0; j < n; j++) { // 寻找第i个节点的邻居节点
                            if (matrix_AdjacencyMatrix[i][j] == 1 && state[j][1] == 0 && random.nextDouble() < a) {
                                newInfections[j] = 1;
                            }
                        }
                    }
