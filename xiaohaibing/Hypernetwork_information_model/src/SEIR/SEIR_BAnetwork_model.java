...
public class SEIR_BAnetwork_model {
    public static void main(String[] args) {
        ...
        try (FileWriter writer = new FileWriter("D:\\桌面\\Hypernetwork_information_model\\output\\SEIR_BAnetwork_model.txt")) {
                ...
                for (int i = 0; i < n; i++) {
                    if (state[i][1] == 0) { // 如果节点在S状态
                        for (int j = 0; j < n; j++) { // 遍历所有节点
                            if (matrix_2section[i][j] == 1 && state[j][1] == 2 && random.nextDouble() < a) {
